...
func (s *Server) handleCreateTunnel(c *gin.Context) {
	...
	if user.MaxTunnels > 0 && int(count) >= user.MaxTunnels {
		c.JSON(http.StatusForbidden, gin.H{
			"success": false,
			"message": fmt.Sprintf("You have reached the maximum tunnel limit (%d) for your current plan. Please upgrade your plan in the Store to create more tunnels.", user.MaxTunnels),
			"code":    "TUNNEL_LIMIT_REACHED",
			"limit":   user.MaxTunnels,
		})
		return
	}
