...
export default function Tunnels() {
  ...
  const handleCreate = async (values: any) => {
    try {
      ...
      if (error instanceof ApiError && error.code === 'TUNNEL_LIMIT_REACHED') {
        Modal.warning({
          title: t('tunnel.limitReached'),
          content: (
            <div>
              <p>{t('tunnel.limitReachedMessage', { limit: error.limit || 5 })}</p>
              <p style={{ marginTop: 12, color: 'var(--text-secondary)' }}>{t('tunnel.limitReachedHint')}</p>
            </div>
          ),
          okText: t('tunnel.goToStore'),
          onOk: () => navigate('/store'),
        })
      } else {
        Modal.error({
